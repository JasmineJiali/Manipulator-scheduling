...
%BufferOut_time(11) Post_time(12) 上一深度状态(13) 14]
...
for i=1:c
    if(m<=28)
    if (state{m,i}(2,1)>400||state{m,i}(2,2)>1100)
        continue;
    end
    elseif(m<=40)
        if (state{m,i}(2,1)>1500||state{m,i}(2,2)>1500)
        continue;
       end
    end
    if (state{m,i}(2,1)>3000||state{m,i}(2,2)>3000||state{m,i}(2,10)>3000|| ...
            state{m,i}(2,11)>3000||state{m,i}(2,12)>3000)
        continue;
    end
    flag=0;
